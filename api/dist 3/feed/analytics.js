...
exports.recordFeedServe = recordFeedServe;
exports.getFeedMetrics = getFeedMetrics;
exports.evaluateRankingPromotion = evaluateRankingPromotion;
...
let pendingFeedServeRows = [];
let feedServeFlushTimer = null;
let feedServeFlushPromise = null;
function numericMetric(value) {
    return typeof value === "number" && Number.isFinite(value) ? value : null;
}
function weightedMetric(value, weight) {
    return value == null ? 0 : value * weight;
}
function missionScore(metrics) {
    if (!metrics)
        return null;
    const scored = weightedMetric(numericMetric(metrics.qualified_reply_rate), 0.32) +
...
function buildHigherBetterCheck(metric, baseline, candidate, threshold, blocker, note) {
...
function buildLowerBetterCheck(metric, baseline, candidate, threshold, blocker, note) {
    if (baseline == null || candidate == null) {
        return {
            metric,
...
function emptyScoreAccumulator() {
    return {
        count: 0,
        Q: [],
        D: [],
...
function finalizeScoreAccumulator(accumulator) {
    return {
...
function timelineKey(viewerId, thoughtId) {
    return `${viewerId}\u0000${thoughtId}`;
}
function recordScore(accumulator, serve) {
    accumulator.count += 1;
    if (typeof serve.scoreQ === "number")
        accumulator.Q.push(serve.scoreQ);
    if (typeof serve.scoreD === "number")
        accumulator.D.push(serve.scoreD);
    if (typeof serve.scoreF === "number")
        accumulator.F.push(serve.scoreF);
    if (typeof serve.scoreR === "number")
        accumulator.R.push(serve.scoreR);
    if (typeof serve.finalRank === "number")
        accumulator.finalRank.push(serve.finalRank);
}
function findLatestServeBefore(timeline, createdAt) {
...
function touchGroupCounter(map, key) {
...
function touchScoreAccumulator(map, key) {
    const current = map.get(key);
    if (current)
        return current;
...
function applyToServeGroups(serve, overall, byBucket, byStage, byPhase, mutate) {
    mutate(overall);
    if (serve.bucket)
        mutate(touchGroupCounter(byBucket, serve.bucket));
    if (serve.stage)
        mutate(touchGroupCounter(byStage, serve.stage));
    if (serve.phaseUsed)
        mutate(touchGroupCounter(byPhase, serve.phaseUsed));
}
function toObject(map) {
    return Object.fromEntries([...map.entries()]);
}
async function recordFeedServe(viewerId, traces, configVersion) {
    if (traces.length === 0)
        return;
    const requestId = (0, node_crypto_1.randomUUID)();
    const servedAt = new Date();
    pendingFeedServeRows.push(...traces.map((trace) => ({
        requestId,
        viewerId,
...
async function flushPendingFeedServes() {
    ...
    feedServeFlushPromise = db_1.db
        .insert(db_1.feedServes)
        .values(rows)
        .then(() => undefined)
        .catch((error) => {
...
async function getFeedMetrics(days = 7) {
    ...
    const serveRows = await db_1.db
        .select()
        .from(db_1.feedServes)
        .where((0, drizzle_orm_1.gte)(db_1.feedServes.servedAt, cutoff))
        .orderBy((0, drizzle_orm_1.asc)(db_1.feedServes.servedAt), (0, drizzle_orm_1.asc)(db_1.feedServes.position));
    const thoughtServeRows = serveRows.filter((row) => row.itemType === "thought" && row.thoughtId);
    const overallCounters = emptyCounters();
    const byBucketCounters = new Map();
    const byStageCounters = new Map();
    const byPhaseCounters = new Map();
    const byConfigVersionCounters = new Map();
    ...
    const timelineByViewerThought = new Map();
    const servesByRequest = new Map();
    for (const serve of thoughtServeRows) {
        ...
        applyToServeGroups(serve, overallCounters, byBucketCounters, byStageCounters, byPhaseCounters, (group) => {
            group.impressions += 1;
        });
        const configCounters = byConfigVersionCounters.get(serve.configVersion) ?? emptyCounters();
    ...
    for (const requestRows of servesByRequest.values()) {
        ...
        for (const serve of requestRows) {
            const isRepeat = serve.authorId != null && recentAuthors.slice(-10).includes(serve.authorId);
            if (isRepeat) {
                applyToServeGroups(serve, overallCounters, byBucketCounters, byStageCounters, byPhaseCounters, (group) => {
    ...
    if (thoughtServeRows.length === 0) {
        return {
            ...
            config: {
                ...
                phase1_viewer_thought_threshold: config_1.feedConfig.phase1ViewerThoughtThreshold,
                phase1_system_engagement_threshold: config_1.feedConfig.phase1SystemEngagementThreshold,
                bucket_ratios: config_1.feedConfig.bucketRatios,
            },
            composition,
    ...
    const replyRows = thoughtIds.length
        ? await db_1.db
            .select()
            .from(db_1.replies)
            .where((0, drizzle_orm_1.and)((0, drizzle_orm_1.inArray)(db_1.replies.thoughtId, thoughtIds), (0, drizzle_orm_1.gte)(db_1.replies.createdAt, cutoff)))
        : [];
    const acceptedReplyIds = replyRows
        .filter((row) => row.status === "accepted")
        .map((row) => row.id);
    const conversationRows = acceptedReplyIds.length
        ? await db_1.db
            .select()
            .from(db_1.conversations)
            .where((0, drizzle_orm_1.inArray)(db_1.conversations.replyId, acceptedReplyIds))
        : [];
    const conversationsByReplyId = new Map(conversationRows.map((conversation) => [conversation.replyId, conversation]));
    const conversationIds = conversationRows.map((conversation) => conversation.id);
    const crossingDraftRows = conversationIds.length
        ? await db_1.db
            .select()
            .from(db_1.crossingDrafts)
            .where((0, drizzle_orm_1.inArray)(db_1.crossingDrafts.conversationId, conversationIds))
        : [];
    const crossingRows = conversationIds.length
        ? await db_1.db
            .select()
            .from(db_1.crossings)
            .where((0, drizzle_orm_1.inArray)(db_1.crossings.conversationId, conversationIds))
        : [];
    ...
    const approvedConversationIds = new Set(crossingRows.map((crossing) => crossing.conversationId));
    for (const reply of replyRows) {
        const timeline = timelineByViewerThought.get(timelineKey(reply.replierId, reply.thoughtId));
        ...
        {
            const configCounter = byConfigVersionCounters.get(serve.configVersion) ?? emptyCounters();
        ...
        if (!crossingStarted && !crossingApproved && !crossingAutoPosted)
            continue;
        applyToServeGroups(serve, overallCounters, byBucketCounters, byStageCounters, byPhaseCounters, (group) => {
            if (crossingStarted) {
    ...
    return {
        ...
        composition,
        overall: finalizeCounters(overallCounters),
        by_bucket: toObject(new Map([...byBucketCounters.entries()].map(([key, value]) => [key, finalizeCounters(value)]))),
        by_stage: toObject(new Map([...byStageCounters.entries()].map(([key, value]) => [key, finalizeCounters(value)]))),
        by_phase: toObject(new Map([...byPhaseCounters.entries()].map(([key, value]) => [key, finalizeCounters(value)]))),
        by_config_version: toObject(new Map([...byConfigVersionCounters.entries()].map(([key, value]) => [
            key,
            finalizeCounters(value),
        ]))),
        score_summary: {
            overall: finalizeScoreAccumulator(overallScores),
            by_bucket: toObject(new Map([...bucketScores.entries()].map(([key, value]) => [key, finalizeScoreAccumulator(value)]))),
        },
    };
}
async function evaluateRankingPromotion(input) {
    const days = Math.min(30, Math.max(1, input.days ?? 7));
    const minimumImpressions = Math.max(25, Math.min(5000, input.minimumImpressions ?? DEFAULT_PROMOTION_MIN_IMPRESSIONS));
    ...
    if ((candidate?.impressions ?? 0) < minimumImpressions) {
        blockers.push(`Candidate config needs at least ${minimumImpressions} impressions before promotion.`);
    }
    if ((baseline?.impressions ?? 0) < minimumImpressions) {
        blockers.push(`Baseline config needs at least ${minimumImpressions} impressions for a fair comparison.`);
    }
    checks.push(buildHigherBetterCheck("qualified_reply_rate", numericMetric(baseline?.qualified_reply_rate), numericMetric(candidate?.qualified_reply_rate), MIN_QUALIFIED_REPLY_DELTA, true, "Candidate must not materially reduce thoughtful replies."), buildHigherBetterCheck("conversation_depth_10_rate", numericMetric(baseline?.conversation_depth_10_rate), numericMetric(candidate?.conversation_depth_10_rate), MIN_DEPTH10_DELTA, true, "Candidate must preserve deep conversation creation."), buildHigherBetterCheck("reply_accept_rate", numericMetric(baseline?.reply_accept_rate), numericMetric(candidate?.reply_accept_rate), MIN_ACCEPT_DELTA, false, "Reply acceptance should stay healthy."), buildHigherBetterCheck("crossing_approval_rate", numericMetric(baseline?.crossing_approval_rate), numericMetric(candidate?.crossing_approval_rate), MIN_ACCEPT_DELTA, false, "Mutual crossings should not trend down."), buildLowerBetterCheck("crossing_autopost_rate", numericMetric(baseline?.crossing_autopost_rate), numericMetric(candidate?.crossing_autopost_rate), MAX_AUTOPOST_DELTA, true, "Autoposted crossings should not rise materially."), buildLowerBetterCheck("repeat_author_exposure_rate", numericMetric(baseline?.repeat_author_exposure_rate), numericMetric(candidate?.repeat_author_exposure_rate), MAX_REPEAT_AUTHOR_DELTA, true, "Repeat author exposure should stay controlled."));
    const candidateScore = missionScore(candidate);
    const baselineScore = missionScore(baseline);
    const scoreDelta = candidateScore != null && baselineScore != null ? candidateScore - baselineScore : null;
    if (scoreDelta == null) {
