...
exports.internalMatchmakerRoutes = internalMatchmakerRoutes;
...
async function internalMatchmakerRoutes(app) {
    ...
    // 2. Recent thoughts
    app.get("/api/internal/matchmaker/thoughts", { preHandler: ensureInternalAccess }, async (request, reply) => {
        const hoursVal = Math.min(168, Math.max(1, parseInt(request.query.hours ?? "48", 10) || 48));
        const limitVal = Math.min(500, Math.max(1, parseInt(request.query.limit ?? "100", 10) || 100));
        ...
        const rows = await db_1.db
            ...
            userPhotoUrl: db_1.users.photoUrl,
            userConcentration: db_1.users.concentration,
        })
            .from(db_1.thoughts)
            .leftJoin(db_1.users, (0, drizzle_orm_1.eq)(db_1.thoughts.userId, db_1.users.id))
            .where((0, drizzle_orm_1.and)((0, drizzle_orm_1.isNull)(db_1.thoughts.deletedAt), (0, drizzle_orm_1.gte)(db_1.thoughts.createdAt, cutoff)))
            .orderBy((0, drizzle_orm_1.desc)(db_1.thoughts.createdAt))
            .limit(limitVal);
        return reply.send({ thoughts: rows });
    });
    // 3. All users
    app.get("/api/internal/matchmaker/users", { preHandler: ensureInternalAccess }, async (_request, reply) => {
        const rows = await db_1.db
            .select({
            id: db_1.users.id,
            name: db_1.users.name,
            photoUrl: db_1.users.photoUrl,
            concentration: db_1.users.concentration,
        })
            .from(db_1.users)
            .orderBy(db_1.users.name);
        return reply.send({ users: rows });
    });
    // 4. Create boost
    app.post("/api/internal/matchmaker/boost", { preHandler: ensureInternalAccess }, async (request, reply) => {
        try {
            ...
            const [targetUser] = await db_1.db
                ...
                .from(db_1.users)
                .where((0, drizzle_orm_1.eq)(db_1.users.id, targetUserId))
                .limit(1);
            if (!targetUser) {
            ...
            const [thought] = await db_1.db
                ...
                .from(db_1.thoughts)
                .where((0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(db_1.thoughts.id, thoughtId), (0, drizzle_orm_1.isNull)(db_1.thoughts.deletedAt)))
                .limit(1);
            if (!thought) {
            ...
            const [existing] = await db_1.db
                .select({ id: db_1.manualBoosts.id })
                .from(db_1.manualBoosts)
                .where((0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(db_1.manualBoosts.targetUserId, targetUserId), (0, drizzle_orm_1.eq)(db_1.manualBoosts.thoughtId, thoughtId), (0, drizzle_orm_1.isNull)(db_1.manualBoosts.consumedAt)))
                .limit(1);
            if (existing) {
                return reply
                    .status(409)
            ...
            const [boost] = await db_1.db
                .insert(db_1.manualBoosts)
                .values({
                targetUserId,
                thoughtId,
            ...
            await (0, service_1.invalidateFeedCache)(targetUserId);
    ...
    // 5. List boosts
    app.get("/api/internal/matchmaker/boosts", { preHandler: ensureInternalAccess }, async (request, reply) => {
        const status = request.query.status ?? "all";
        const limitVal = Math.min(200, Math.max(1, parseInt(request.query.limit ?? "50", 10) || 50));
        const conditions = [];
        if (status === "pending") {
            conditions.push((0, drizzle_orm_1.isNull)(db_1.manualBoosts.consumedAt));
        }
        else if (status === "consumed") {
            conditions.push((0, drizzle_orm_1.sql) `${db_1.manualBoosts.consumedAt} IS NOT NULL`);
        }
        const rows = await db_1.db
            .select({
            id: db_1.manualBoosts.id,
            targetUserId: db_1.manualBoosts.targetUserId,
            thoughtId: db_1.manualBoosts.thoughtId,
            createdBy: db_1.manualBoosts.createdBy,
            reason: db_1.manualBoosts.reason,
            consumedAt: db_1.manualBoosts.consumedAt,
            createdAt: db_1.manualBoosts.createdAt,
            thoughtSentence: db_1.thoughts.sentence,
            targetUserName: db_1.users.name,
        })
            .from(db_1.manualBoosts)
            .leftJoin(db_1.thoughts, (0, drizzle_orm_1.eq)(db_1.manualBoosts.thoughtId, db_1.thoughts.id))
            .leftJoin(db_1.users, (0, drizzle_orm_1.eq)(db_1.manualBoosts.targetUserId, db_1.users.id))
            .where(conditions.length > 0 ? (0, drizzle_orm_1.and)(...conditions) : undefined)
            .orderBy((0, drizzle_orm_1.desc)(db_1.manualBoosts.createdAt))
            .limit(limitVal);
        return reply.send({ boosts: rows });
    });
    // 6. Cancel (delete) a pending boost
    app.delete("/api/internal/matchmaker/boost/:id", { preHandler: ensureInternalAccess }, async (request, reply) => {
        ...
        const [boost] = await db_1.db
            ...
            .from(db_1.manualBoosts)
            .where((0, drizzle_orm_1.eq)(db_1.manualBoosts.id, id))
            .limit(1);
        if (!boost) {
        ...
        await db_1.db.delete(db_1.manualBoosts).where((0, drizzle_orm_1.eq)(db_1.manualBoosts.id, id));
