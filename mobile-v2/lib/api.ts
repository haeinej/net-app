...
export async function loginWithGoogle(idToken: string): Promise<AuthResponse> {
  return requestJson<AuthResponse>("/api/auth/google", "Google sign in failed", {
    method: "POST",
    headers: JSON_HEADERS,
...
export async function linkSocialAccount(
  ...
  return requestJson<{ linked: string }>("/api/auth/link-social", "Could not link account", {
    method: "POST",
    auth: true,
    headers: JSON_HEADERS,
...
export async function fetchFriends(): Promise<FriendUser[]> {
  const data = await requestJson<{ friends: any[] }>("/api/friends", "Friends failed", { auth: true });
  return (data?.friends ?? []).map((f: any) => ({
    id: f.user?.id ?? "",
    name: f.user?.name ?? null,
    handle: f.user?.handle ?? null,
    photo_url: f.user?.photo_url ?? null,
  }));
}
...
export async function fetchFriendsFeed(
  ...
  let query = `/api/feed?friends_only=true&limit=${limit}`;
  if (cursor) query += `&cursor=${encodeURIComponent(cursor)}`;
  const data = await requestJson<unknown>(query, "Friends feed failed", { auth: true });
  return normalizeFeedPageResponse(data);
}
...
export async function sendFriendRequest(
  ...
  await requestVoid("/api/friends/request", "Friend request failed", {
    auth: true,
    method: "POST",
    headers: JSON_HEADERS,
    body: JSON.stringify({ targetUserId, thoughtId, replyText }),
  });
}

export async function acceptFriendRequest(friendshipId: string): Promise<void> {
  await requestVoid("/api/friends/accept", "Accept failed", {
    auth: true,
    method: "POST",
    headers: JSON_HEADERS,
...
export async function removeFriend(userId: string): Promise<void> {
  await requestVoid(`/api/friends/${encodeURIComponent(userId)}`, "Unfriend failed", {
    auth: true,
    method: "DELETE",
  });
}
...
export async function saveCard(thoughtId: string): Promise<void> {
  await requestVoid("/api/saved", "Save failed", {
    auth: true,
    method: "POST",
    headers: JSON_HEADERS,
...
export async function unsaveCard(thoughtId: string): Promise<void> {
  await requestVoid(`/api/saved/${encodeURIComponent(thoughtId)}`, "Unsave failed", {
    auth: true,
    method: "DELETE",
  });
}

export async function fetchSavedCards(limit = 20): Promise<FeedItem[]> {
  const data = await requestJson<{ items: unknown[] }>(
    `/api/saved?limit=${limit}`,
    "Saved cards failed",
    { auth: true }
  );
  return (data?.items ?? [])
...
export async function submitFeedback(questionId: string, response: string): Promise<void> {
  await requestVoid("/api/feedback", "Feedback failed", {
    auth: true,
    method: "POST",
    headers: JSON_HEADERS,
...
export async function trackEngagement(events: EngagementEvent[]): Promise<void> {
  await requestVoid("/api/engagement/track", "Could not track engagement", {
    auth: true,
    method: "POST",
    headers: JSON_HEADERS,
