...
exports.scoreThought = scoreThought;
const embedding_1 = require("../embedding");
function maxCosineSim(thoughtVec, viewerVecs) {
    if (!thoughtVec || viewerVecs.length === 0)
        return 0;
    let max = 0;
    for (const v of viewerVecs) {
        if (v.length !== thoughtVec.length)
            continue;
        const sim = (0, embedding_1.cosineSimilarity)(thoughtVec, v);
        if (sim > max)
            max = sim;
    }
    return max;
...
function getSimilarities(thought, viewerEmbeddings) {
    const resonance_similarity = thought.resonanceEmbedding && viewerEmbeddings.resonanceEmbeddings.length > 0
        ? maxCosineSim(thought.resonanceEmbedding, viewerEmbeddings.resonanceEmbeddings)
        : 0;
    const surface_similarity = thought.surfaceEmbedding && viewerEmbeddings.surfaceEmbeddings.length > 0
        ...
        : thought.surfaceEmbedding && viewerEmbeddings.interestsEmbedding
            ? (0, embedding_1.cosineSimilarity)(thought.surfaceEmbedding, viewerEmbeddings.interestsEmbedding)
            : 0;
 ...
 */
function scoreThought(thought, viewerEmbeddings, viewerProfile, alpha) {
    const quality = Math.min(1, Math.max(0, thought.qualityScore ?? 0.5));
    const hasViewerResonance = viewerEmbeddings.resonanceEmbeddings.length > 0 &&
        viewerEmbeddings.resonanceEmbeddings.some((q) => q.length > 0);
    if (!hasViewerResonance && viewerEmbeddings.interestsEmbedding) {
        const interestsSim = thought.surfaceEmbedding && viewerEmbeddings.interestsEmbedding
            ? (0, embedding_1.cosineSimilarity)(thought.surfaceEmbedding, viewerEmbeddings.interestsEmbedding)
            : 0;
